...
class EnhancedBookGeneratorWithImages:
    def __init__(self, genai_api_key: str, search_api_key: str, cse_id: str):
        ...
        # CRITICAL FIX: Realistic words per page for PDF output
        self.words_per_pdf_page = 180  # More realistic for actual PDF pages
    ...
    def calculate_target_pages(self, settings: BookSettings) -> int:
        """Calculate actual target PDF pages based on book length selection"""
        length_targets = {
            "Short Book": 75,      # Target 75 PDF pages
            "Standard Book": 200,  # Target 200 PDF pages  
            "Extended Book": 350,  # Target 350 PDF pages
            "Epic Book": 550       # Target 550 PDF pages
        }
        return length_targets.get(settings.book_length, 200)
    
    def get_user_inputs(self) -> BookSettings:
        """Interactive function to get all user inputs with realistic page estimates"""
        print("=" * 60)
        ...
        # Book length with REALISTIC PDF page estimates
        print("\n📏 STEP 4: Book Length (PDF Page Count)")
        lengths = [
            ("Short Book", "60-100 PDF pages"),
            ("Standard Book", "150-250 PDF pages"),
            ("Extended Book", "300-400 PDF pages"),
            ("Epic Book", "500+ PDF pages")
        ]
        ...
        # Calculate target PDF pages
        target_pdf_pages = self.calculate_target_pages(BookSettings(
            concept=concept, genre=selected_genre, target_audience=selected_audience,
            book_length=selected_length, tone=tone, complexity=complexity, perspective=perspective
        ))
        
        # Book structure based on TARGET PDF PAGES
        print(f"\n🏗️ STEP 6: Book Structure (Target: {target_pdf_pages} PDF pages)")
        while True:
            try:
                chapters_count = int(input("How many chapters do you want? (5-25): "))
                if 5 <= chapters_count <= 25:
                    break
                else:
                    print("Please enter a number between 5-25")
            except ValueError:
                print("Please enter a valid number")
        
        while True:
            try:
                sections_per_chapter = int(input("How many sections per chapter? (3-12): "))
                if 3 <= sections_per_chapter <= 12:
                    break
                else:
        ...
        # Calculate pages per section to hit target PDF pages
        total_sections = chapters_count * sections_per_chapter
        pages_per_section = max(1, target_pdf_pages // total_sections)
        
        print(f"📄 Calculated pages per section: {pages_per_section} (to reach {target_pdf_pages} PDF pages)")
        adjust_pages = input(f"Adjust pages per section? Current: {pages_per_section} (press Enter to keep, or enter new number): ")
        if adjust_pages.isdigit():
            pages_per_section = int(adjust_pages)
        ...
        # Display summary with REALISTIC PDF page estimate
        estimated_pdf_pages = chapters_count * sections_per_chapter * pages_per_section
        total_words = estimated_pdf_pages * self.words_per_pdf_page
        ...
        print(f"📄 TARGET PDF Pages: {estimated_pdf_pages}")
        print(f"📝 Estimated Total Words: {total_words:,}")
        print(f"🖼️  Include Images: {'Yes' if include_images else 'No'}")
        print("=" * 60)
        
        confirm = input("\n⚠️  This will generate a COMPLETE book targeting the specified PDF page count. Continue? (y/n): ").lower()
        if confirm != 'y':
    ...
    def generate_book_structure(self, settings: BookSettings) -> Dict:
        ...
        # Calculate realistic PDF page targets
        target_pdf_pages = settings.chapters_count * settings.sections_per_chapter * settings.pages_per_section
        target_words_per_section = settings.pages_per_section * self.words_per_pdf_page
        
        prompt = f"""
        Create a detailed structure for a {settings.book_length.lower()} titled "{settings.book_title}".
        TARGET: {target_pdf_pages} PDF pages with approximately {target_words_per_section * settings.chapters_count * settings.sections_per_chapter:,} total words.
        ...
        - Words per section: {target_words_per_section}
        ...
        5. Estimated word count: {target_words_per_section * settings.sections_per_chapter} words per chapter
    ...
    def elaborate_chapter(self, chapter_number: int, chapter_title: str, sections_list: List[str]) -> Dict:
        ...
        # Use realistic words per PDF page calculation
        words_per_section = settings.pages_per_section * self.words_per_pdf_page
        total_chapter_words = len(sections_list) * words_per_section
        ...
        Target Audience: {settings.target_audience}
        CRITICAL: Each section must contain approximately {words_per_section} words to achieve proper PDF page count.
        ...
        For each section, write EXACTLY {words_per_section} words including:
        - Clear section heading with markdown formatting (## Section Title)
        - Comprehensive introduction paragraph (150-200 words)
        - Main content with detailed explanations (60% of section words)
        - Multiple practical examples and applications (20% of section words)
        - Code examples, diagrams, or technical details where relevant (15% of section words)
        - [IMAGE PLACEHOLDER] markers where visual aids would be helpful
        - Detailed summary or key takeaways (5% of section words)
        - Smooth transition to next section
        
        Include [IMAGE PLACEHOLDER: description] markers in the text where images would enhance understanding.
        
        IMPORTANT REQUIREMENTS:
        - Each section MUST be approximately {words_per_section} words (no exceptions)
        - Content must be comprehensive and detailed for {settings.target_audience}
        - Written in {settings.tone} tone throughout
        - Technically accurate and informative
        - Well-structured with proper subheadings
        ...
        TOTAL TARGET WORD COUNT FOR THIS CHAPTER: {total_chapter_words} words
        
        Remember: This is for a {settings.book_length} targeting extensive content, so be thorough and detailed.
        """
    ...
    def create_pdf_export(self, filename: str = None) -> bool:
        ...
        try:
            ...
            # Create PDF document with tighter spacing for more content per page
            doc = SimpleDocTemplate(filename, pagesize=A4, 
                                  topMargin=60, bottomMargin=60,   # Reduced margins
                                  leftMargin=60, rightMargin=60)   # Reduced margins
            
            # Define styles
            styles = getSampleStyleSheet()
            
            # Custom styles optimized for more content per page
            ...
            total_words = sum(len(chapter['content'].split()) for chapter in self.book_data.get('chapters', {}).values())
            ...
            stats_data = [
                ...
                ['TARGET PDF Pages', str(target_pdf_pages)],
                ['Total Words Generated', f'{total_words:,}'],
                        ...
                        if paragraph.strip():
                            ...
                            if paragraph.strip().startswith('##'):
                                ...
                                if image_index < len(images) and os.path.exists(images[image_index]['filename']):
                                    try:
                                        img = ReportLabImage(images[image_index]['filename'], width=3*inch, height=2.25*inch)
...
def main():
    ...
    try:
        ...
        if structure_result['success']:
            ...
            target_pdf_pages = settings.chapters_count * settings.sections_per_chapter * settings.pages_per_section
            print(f"📊 TARGET PDF Pages: {target_pdf_pages}")
            ...
            if pdf_success:
                ...
                print(f"📄 TARGET PDF Pages: {target_pdf_pages}")
                print(f"📄 Actual PDF Pages: Check the generated PDF file")
